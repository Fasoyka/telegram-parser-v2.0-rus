# telegram-parser-v2.0. Парсер чатов и рассылщик сообщений (Telethon)
...
* загружать новые списки пользователей через `.txt` файл или `.zip` архив,
...
Списки пользователей хранятся в папке `lists`. Команда `/lists` показывает доступные списки, `/users <имя>` отправляет выбранный файл, а `/add_list` позволяет загрузить новый список в формате `.txt` или `.zip`.
